...
public class DistributedLockAspectConfiguration {
	...
	@Pointcut("@annotation(com.tom.lock.redis.spring.boot.autoconfigure.annotations.RedisAction)")
	...
	public Object around(ProceedingJoinPoint pjp) throws Throwable{
		...
		RedisAction lockAction = method.getAnnotation(RedisAction.class);
		String key = lockAction.value();
		Object[] args = pjp.getArgs();
		key = parse(key, method, args);
		
		
		int retryTimes = lockAction.action().equals(RedisAction.LockFailAction.CONTINUE) ? lockAction.retryTimes() : 0;
